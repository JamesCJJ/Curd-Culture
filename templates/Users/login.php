<?php
/**
 * Sign in page with Google reCAPTCHA v2 (checkbox).
 * $siteKey is set by UsersController from Security.recaptcha.site_key.
 */
...
<div class="auth-page">
    <div class="auth-card">
        ...
        <!-- reCAPTCHA widget (renders only when siteKey is available) -->
...
<?php
// EITHER use the block system (requires your layout to call $this->fetch('script')):
$this->Html->script('https://www.google.com/recaptcha/api.js', ['block' => true]);

// OR output immediately (simpler, avoids the “block” requirement):
...
<style>
    ...
    .auth-card{ width:100%; max-width:440px; padding:1.35rem 1.2rem 1.25rem; border-radius:1.05rem; background:#fff; border:1px solid #eef0f3; box-shadow:0 12px 36px rgba(2,6,23,.08); }
    ...
    .auth-input{ display:block; width:100%; padding:12px 14px; font-size:15px; border-radius:.7rem; border:none; outline:none; box-shadow: inset 0 1px 2px rgba(0,0,0,.05); background:#f9fafb; color:#111827; transition:all .2s ease-in-out; }
